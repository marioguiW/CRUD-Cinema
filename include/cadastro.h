...
// exibe um menu de opções para o usuário e retorna a opção escolhida.
int cadastro()
{
...
// são responsáveis por ler o último ID cadastrado de Cinema.
int ler_ultimo_id_cinema()
{
    ...
    while (fgets(line, 100, cinema) != NULL)
    {
        ...
        { // compara se os primeiros 4 caracteres da linha são == "id :"
            int id;
            if (sscanf(line, "id : %d", &id) == 1)
            {                   // sscanf lê o elemento de uma array e incrementa em outra
...
// Aqui segue as estruturas do cliente e do filme.
typedef struct
{
    int id;
    char *nome_filme;
...
// recebe um arquivo de texto (referente aos clientes cadastrados)
